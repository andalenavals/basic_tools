...
def parse_args():
    ...
    # Drectory arguments
    parser.add_argument('--inpath', default='/home2/dfa/sobreira/alsina/catalogs/output/y3a1-v29/byzone/',
...
def overall_rho(work, zonenum,  outpath):
    import numpy as np
    ...
    for key in keys:
        stat_file = os.path.join(work + 'zone_' + str(zonenum), "rho_" + key + ".json")
        ...
        formats = ['i4' ,'f8' ,'f8' ,'f8' ]
        values =  [int(zonenum), np.nanmean(rho2p), np.nanmean(rho2m),  np.nanmean(sig_rho2)]
        ...
        file_name_out = "y3a1-v29_rho2byzone.fits"
