...
def parse_args():
    ...
    # Drectory arguments
    parser.add_argument('--inpath', default='/home2/dfa/sobreira/alsina/catalogs/output/y3a1-v29/byccd/all/',
...
def overall_rho(work, ccdnum,  outpath):
    import numpy as np
    ...
    for key in keys:
        stat_file = os.path.join(work + 'ccd_' + str(ccdnum), "rho_" + key + ".json")
        ...
        formats = ['i4' ,'f8' ,'f8' ,'f8' ]
        values =  [int(ccdnum), np.nanmean(rho2p), np.nanmean(rho2m),  np.nanmean(sig_rho2)]
        #values =  [int(ccdnum), rho2p[25], rho2m[25],  sig_rho2[25]]
        print(values)
        ...
        file_name_out = "y3a1-v29_rho2byccd.fits"
...
def main():
    ...
    for ccdnum in range(1, 63):
        overall_rho(work, ccdnum,  outpath)
